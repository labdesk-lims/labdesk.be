CREATE TABLE [dbo].[request_material] (
    ...
    [amount]   INT            CONSTRAINT [DF_request_material_amount] DEFAULT ((0)) NOT NULL,
    [comment]  NVARCHAR (MAX) NULL,
    [request]  INT            NOT NULL,
    CONSTRAINT [PK_request_material] PRIMARY KEY CLUSTERED ([id] ASC),
    CONSTRAINT [FK_request_material_material] FOREIGN KEY ([material]) REFERENCES [dbo].[material] ([id]),
    CONSTRAINT [FK_request_material_request] FOREIGN KEY ([request]) REFERENCES [dbo].[request] ([id]) ON DELETE CASCADE
);
