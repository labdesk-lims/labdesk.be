CREATE TABLE [dbo].[task_material] (
    ...
    [amount]           INT            CONSTRAINT [DF_project_material_amount] DEFAULT ((0)) NOT NULL,
    [comment]          NVARCHAR (MAX) NULL,
    [task]             INT            NOT NULL,
    [billing_customer] INT            NULL,
    CONSTRAINT [PK_project_material] PRIMARY KEY CLUSTERED ([id] ASC),
    CONSTRAINT [FK_project_material_task] FOREIGN KEY ([task]) REFERENCES [dbo].[task] ([id]) ON DELETE CASCADE,
    CONSTRAINT [FK_task_material_billing_customer] FOREIGN KEY ([billing_customer]) REFERENCES [dbo].[billing_customer] ([id]) ON DELETE SET NULL
);
